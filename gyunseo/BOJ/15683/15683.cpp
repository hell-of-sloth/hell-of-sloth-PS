...
void update_matrix(int i, int j, int dir) {
    ...
    while (iteration_trigger) {
        ...
        if (matrix[ni][nj] >= 1 && matrix[ni][nj] <= 5) continue;
        // 만약에 [ni][nj]에 미리 누가 마킹을 했다면 그냥 지나가기
        if (matrix[ni][nj] == MARK) continue;
...
void solve() {
    ...
    for (int brute_case = 0; brute_case < brute_cases; brute_case++) {
        ...
        for (int i = 0; i < num_cam; i++) {
            ...
            ASSERT(cur_cam_type >= 1 && cur_cam_type <= 5, "cur_cam_type is out of bound!");
            ASSERT(cur_dir >= UP && cur_dir <= LEFT, "cur_dir is out of bound!");
            tmp_case = tmp_case / 4;
            switch (cur_cam_type) {
                case 1:
...
void read_user_input() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            cin >> MATRIX[i][j];
            if (MATRIX[i][j] >= 1 && MATRIX[i][j] <= 5) cam_poses.push_back({i, j});
        }
    }
}
