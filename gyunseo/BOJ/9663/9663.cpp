...
bool is_in_diagonal_path(int cur_col, int cur_row) {
    ...
    for (int i = 0; i < num_issued_row; i++) {
        ...
        if (abs(cur_row - row) == abs(cur_col - col)) return true; 
    }
    return false;
}
void dfs(int col, int row) {
    ...
    for (int i = 0; i < N; i++) {
        if (find(is_issued_col.begin(), is_issued_col.end(), i) != is_issued_col.end()) continue;
        if (is_in_diagonal_path(i, row + 1)) continue;
